...
const demoUsers = {
  customer: { email: "asiakas@demo.fi", password: "demo123", approved: true },
  courier: { email: "lahetti@demo.fi", password: "demo123", approved: true },
  merchant: { email: "ravintola@demo.fi", password: "demo123", approved: true },
  admin: { email: "admin@demo.fi", password: "demo123", approved: true },
};
...
function renderAuthPanels() {
  ...
  qs("authPanels").innerHTML = `
    <div class="auth-panel">
      ...
      ${state.selectedRole !== "customer" && state.selectedRole !== "admin" ? `
      <details>
        <summary>Uusi ${role.label.toLowerCase()}tili</summary>
        <form id="signupForm">
          <label>Nimi
          ...
          <button type="submit">Lähetä adminille hyväksyttäväksi</button>
        </form>
      </details>` : ""}
  ...
  if (signup) signup.addEventListener("submit", signupForApproval);
}
...
async function login(event) {
  ...
  if (state.firebase.ready) {
    try {
      const { signInWithEmailAndPassword } = state.firebase.fns;
      const cred = await signInWithEmailAndPassword(state.firebase.auth, email, password);
      state.session = { role: state.selectedRole, email: cred.user.email, uid: cred.user.uid };
  ...
  const user = demoUsers[state.selectedRole];
  if (email !== user.email || password !== user.password) {
    qs("authMessage").textContent = "Virheellinen tunnus tai salasana valitulle roolille.";
    return;
  }

  state.session = { role: state.selectedRole, email };
...
async function oauthLogin(providerName) {
  ...
  try {
    const { signInWithPopup, GoogleAuthProvider, OAuthProvider } = state.firebase.fns;
    ...
    state.session = { role: state.selectedRole, email: result.user.email || "oauth-user", uid: result.user.uid };
...
async function signupForApproval(event) {
  event.preventDefault();
  const name = qs("signupName").value.trim();
  const email = qs("signupEmail").value.trim();

  const item = { id: crypto.randomUUID(), role: state.selectedRole, name, email, status: "pending" };
  ...
  if (state.firebase.ready) {
    ...
    qs("authMessage").textContent = "Hakemus lähetetty Firestoreen adminin hyväksyttäväksi.";
  } else {
    state.queue.push(item);
    localStorage.setItem("approvalQueue", JSON.stringify(state.queue));
    qs("authMessage").textContent = "Hakemus lähetetty (demo-localStorage).";
  }
...
async function placeOrder() {
  ...
  if (state.firebase.ready) {
    const { addDoc, collection, serverTimestamp } = state.firebase.fns;
    await addDoc(collection(state.firebase.db, "orders"), {
      ...orderPayload,
      createdAt: serverTimestamp(),
    });
  }
